exports.handle = async function(args, message, client) {
  ...
  await channel.send('Type `!bill scan [state name/code]` to scan for new bills in a given state.');
  await channel.send('Type `!bill add [bill id]` to add an unkown bill to the watchlist.');
  await channel.send('Type `!bill watch/ignore [state name/code] [bill #]` to add or remove a known bill from the watchlist.');
